;(function(){
	...
	module.map=(function(){
		...
		function _createElement(){
			...
			div.id="global-map";
			div.className="global-block global-map";
			div.innerHTML='<div id="allmap" class="allmap"></div>\
				<div class="map-btm"></div>\
				<div class="btn-closer"></div>';
			document.body.appendChild(div);
		}
		return {
			doFn:function(id){
				typeof(fn[id])=='function'&&fn[id](map);
			},
			...
			init:function(_city){
				...
				ev.forEach(ev.getAll('.btn-map'),function(item,i){
					ev.addEvent(item,'click',function(){
						ev.get('#global-map').classList.add('show');
				...
				ev.addEvent(ev.get('#global-map .btn-closer'),'click',function(){
